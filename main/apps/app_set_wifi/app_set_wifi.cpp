...
#include <WiFi.h>
#include "../utils/wifi/wifi_auto.h"
#include "esp_sntp.h"
...
{
    // define _wifi_ssid and _wifi_password as static variables
    static char *_wifi_ssid;
    static char *_wifi_password;

    // initialize _wifi_ssid and _wifi_password
    _wifi_ssid = new char[50];
    _wifi_password = new char[50];

    // get the ssid from eeprom using the function wifi_auto_get_ssid(*ssid) and store it into _wifi_ssid pointer
    Preferences preferences;
    preferences.begin("wifi", true);
    int ssid_len = preferences.getString("ssid", _wifi_ssid, SSID_SIZE);
    int pwd_len = preferences.getString("password", _wifi_password, PASSWORD_SIZE);
    preferences.end();
    ...
    if (_data.current_state == state_init)
    {
        ...
        // if _wifi_ssid is not null pointer
        if (ssid_len > 0)
        {
    ...
    if (_data.current_state == state_connect)
    {
        ...
        {
            wifi_auto_connect(_data.wifi_ssid.c_str(), _data.wifi_password.c_str());
        }
